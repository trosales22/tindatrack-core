...
const BusinessCard: React.FC<BusinessCardProps> = ({ businessId, businessDetail }) => {
  ...
  const { openEditBusiness, openDeleteBusiness, setOpenEditBusiness, setOpenDeleteBusiness } =
    useModalStore();
    ...
    <>
      <div className="bg-white shadow-md rounded-2xl p-4 mb-4">
        <div className="flex justify-between items-center">
          <h1
            className="text-xl sm:text-2xl font-bold cursor-pointer flex items-center gap-2"
          ...
          <div className="flex gap-3">
            <button
              ...
              onClick={() => setOpenEditBusiness(true)}
            >
        ...
        {isOpen && (
          <div className="mt-4">
            ...
            <p className="text-sm sm:text-sm text-gray-800">
              Owner: {`${businessDetail?.owner?.firstname} ${businessDetail?.owner?.lastname}`}
            </p>
            <p className="text-sm sm:text-sm text-gray-800">
              Status: {businessDetail?.status?.label}
            </p>
            <p className="text-sm sm:text-sm text-gray-800">
              Created on: {businessDetail?.created_at}
            </p>
          </div>
        )}
      </div>
        ...
        {openDeleteBusiness && (
          <>
            ...
            <div className="flex justify-end space-x-2 mt-4">
              <Button variant="ghost" onClick={() => setOpenDeleteBusiness(false)}>
                No
              </Button>
              <Button
                variant="danger"
              ...
              >
                {isDeleteBusinessLoading ? 'Deleting..' : 'Yes'}
              </Button>
            </div>
          </>
        )}
