...
const BDProductSection: React.FC<ProductSectionProps> = ({ businessId }) => {
  ...
  const {
    ...
    isLoading,
    isError,
  }: any = useListBusinessProductQuery({
    businessId,
              ...
              return (
                <tr key={item.id}>
                  ...
                  <td className="font-medium">
                    {formatCurrency(item?.attributes?.unit_price || 0.0)}
                  </td>
                  <td className="font-medium">
                    {formatCurrency(item?.attributes?.cost_price || 0.0)}
                  </td>
        ...
        {openDeleteBusinessProduct && (
          <>
            ...
            <div className="flex justify-end space-x-2 mt-4">
              <Button variant="ghost" onClick={() => setOpenDeleteBusinessProduct(false)}>
                No
              </Button>
              <Button
                variant="danger"
              ...
              >
                {isDeleteProductLoading ? 'Deleting..' : 'Yes'}
              </Button>
            </div>
          </>
        )}
